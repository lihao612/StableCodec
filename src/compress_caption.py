...
from my_utils.testing_utils_caption import parse_args_testing
...
def load_captions(caption_path):
    ...
    if not caption_path.exists():
        raise FileNotFoundError(f"[Caption] Caption file not found: {caption_path}")

    with caption_path.open("r", encoding="utf-8") as f:
        captions = json.load(f)

    print(f"[Caption] Loaded {len(captions)} captions from {caption_path}")
    return captions


def resolve_prompt(image_name, captions):
    prompt = captions.get(image_name)
    if prompt is None:
        raise KeyError(f"[Caption] Missing caption for {image_name}")
    return [prompt]
...
def main(args):
    ...
    bpp = []
    images = sorted(glob.glob(os.path.join(args.img_path, '*.png')))
    ...
    for img_path in images:
        ...
        prompt = resolve_prompt(name, captions)
        ...
        with torch.no_grad():
            try:
                ...
                out_img = decompress_one_image(net, args.bin_path, ori_h, ori_w, fname, prompt)
