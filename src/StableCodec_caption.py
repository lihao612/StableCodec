...
class StableCodec(torch.nn.Module):
    ...
    def _normalize_prompt_texts(self, pos_prompt, batch_size):
        if isinstance(pos_prompt, str):
            return [pos_prompt for _ in range(batch_size)]

        if not isinstance(pos_prompt, (list, tuple)):
            raise TypeError("pos_prompt must be a string or a list/tuple of strings")

        if len(pos_prompt) == 0:
            raise ValueError("pos_prompt must not be empty")
        ...
        return list(pos_prompt)

    def _encode_prompt_texts(self, prompt_texts):
        with torch.no_grad():
    ...
    def _get_prompt_embeddings(self, pos_prompt, batch_size, device):
        prompt_texts = self._normalize_prompt_texts(pos_prompt, batch_size)

        # print(f"[Prompt]: {prompt_texts}")
        # print(f"[Prompt]: {len(prompt_texts)} prompts for batch size {batch_size}")

        return self._encode_prompt_texts(prompt_texts).to(device)
    ...
    def forward(self, x, pos_prompt, ori_h, ori_w):
        ...
        with torch.no_grad():
            ...
            pos_caption_enc = self._get_prompt_embeddings(pos_prompt, x.shape[0], x.device)
